...
ZIP_FILE="erp_every-1-days_${date}.zip"

RDS_HOST="localhost"
FILESTORE_SRC="/opt/odoo/.local/share/Odoo/filestore/erp"
EVIDENCE_FILE="$BACKUP_DIR/erp14_evidence.txt"
S3_DEST="s3://backups-trial360/erp14-integra/daily/"
...
"Base de datos: $DB_NAME
Fecha: $date
Servidor: ERP14 Integra
Destino: $S3_DEST"

log "Iniciando backup $DB_NAME"

#######################################
# Dump de base de datos (LOCAL)
#######################################
log "Ejecutando pg_dump local"
sudo pg_dump \
  -U "$ODOO_USER" \
  -h "$RDS_HOST" \
  -Fp \
